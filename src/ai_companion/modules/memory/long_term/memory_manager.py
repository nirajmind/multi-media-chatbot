...
class MemoryManager:
    ...
    async def _analyze_memory(self, message: str) -> MemoryAnalysis:
        ...
        self.logger.info(f"Headers: {headers_to_log}")
        request_body = {
            "model": self.model,
        ...
        if "messages" in body_to_log and body_to_log["messages"] and isinstance(body_to_log["messages"][0], dict) and "content" in body_to_log["messages"][0]:
            ...
            self.logger.info(f"Request prompt: {body_to_log['messages'][0]['content']}")
        else:
            self.logger.info("Request body (without sensitive info): <redacted>")
        ...
        async with httpx.AsyncClient(
            ...
            try:
                ...
                self.logger.error(f"Request body: {e.request.content}")
                raise
    ...
    async def extract_and_store_memories(self, message: BaseMessage) -> None:
        ...
        if analysis.is_important and analysis.formatted_memory:
            ...
            self.logger.info(f"Storing new memory: '{analysis.formatted_memory}' with metadata: {metadata}")
